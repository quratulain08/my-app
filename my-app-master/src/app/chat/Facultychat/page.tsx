...
const mockMessages: Record<string, Array<{ id: number; student: string; message: string }>> = {
  '1': [
    ...
    { id: 2, student: 'Faculty', message: 'Sure, what would you like to know?' },
  ],
  '2': [
    { id: 3, student: 'Jane Smith', message: 'Can you provide feedback on my recent activity log?' },
    { id: 4, student: 'Faculty', message: 'I will review it and get back to you.' },
  ],
};
  ...
  const handleSendMessage = () => {
    if (newMessage.trim() && selectedStudent) {
      setMessages(prevMessages => [
        ...prevMessages,
        { id: prevMessages.length + 1, student: 'Faculty', message: newMessage }
      ]);
      setNewMessage('');
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f7f7f7]">
      ...
      <div className="bg-[#112d60] text-white p-4 flex items-center shadow-md">
        <Link href="/Faculty" className="text-white mr-4">
            ...
            <>
              <div className="flex-1 p-4 overflow-auto bg-gray-100">
                <div className="bg-white p-4 rounded-lg shadow-lg max-h-[500px]">
                  {messages.map((msg) => (
                    <div key={msg.id} className={`mb-2 ${msg.student === 'Faculty' ? 'text-right' : ''}`}>
                      <div className={`p-3 rounded-lg ${msg.student === 'Faculty' ? 'bg-[#112d60] text-white' : 'bg-gray-200'}`}>
